...
PROJECT_ROOT_DB = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", DB_FILENAME))
        ...
        if os.path.exists(LOCAL_DB_PATH):
            try:
                ...
                if local_mtime >= remote_mtime:
                    print(f"Cloud: Using cached database (Local: {local_mtime}, Remote: {remote_mtime})")
                    should_download = False
            except Exception as meta_err:
                print(f"Cloud: Metadata check failed, falling back to cached file if exists: {meta_err}")
...
def api(req: https_fn.Request) -> https_fn.Response:
    ...
    if "file" in path:
        if not IS_EMULATOR:
            return https_fn.Response("File proxy only available in emulator", status=403)
    ...
    if "search" not in path:
        return https_fn.Response(f"Not Found", status=404)
    ...
    try:
        ...
        for row in results:
            ...
            start = max(0, match_pos - 30); end = min(len(content), match_pos + 50)
            ...
            if IS_EMULATOR:
                ...
                target_bucket = GCS_BUCKET_NAME if source == "association" else GCS_INTERNAL_BUCKET_NAME
                file_url = f"https://storage.googleapis.com/{target_bucket}/{row['filename']}"
        ...
        conn.close()
        import json
        return https_fn.Response(json.dumps(output, ensure_ascii=False), mimetype="application/json")
    except Exception as e:
