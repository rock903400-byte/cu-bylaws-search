...
def get_category(filename):
    categories = {
        "法": "法律", "辦法": "辦法", "要點": "要點", "規則": "規則",
        "章程": "章程", "簡則": "簡則", "須知": "作業須知", "手冊": "手冊", "範例": "範例"
    }
    for kw, cat in categories.items():
        if kw in filename: return cat
    return "其他"
...
def index_source(conn, source_name):
    ...
    if source_name == "internal":
        dirs_to_scan.extend([os.path.join(SOURCE_DIR, "internal"), os.path.join(SOURCE_DIR, "內部法規")])
    else:
    ...
    for d in dirs_to_scan:
        if os.path.exists(d):
            try:
                files = [f for f in os.listdir(d) if f.lower().endswith(('.pdf', '.docx')) and os.path.isfile(os.path.join(d, f))]
                for f in files:
    ...
    for file_dir, filename in all_files:
        ...
        print(f"[{source_name}] [{indexed_count+1}/{len(all_files)}] Indexing: {filename}")
